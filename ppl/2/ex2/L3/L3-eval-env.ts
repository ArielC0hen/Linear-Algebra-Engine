...
import { isClosure, makeClosureEnv, Closure, Value, makeClassValueEnv, ClassValue, makeObjectValue, makeObjectValueEnv, isClassValue, isObjectValue, ObjectValue, isSymbolSExp } from "./L3-value";
...
const applyObjectMethod = (proc: ObjectValue, args: Value[]): Result<Value> => {
    ...
    const methodBinding = proc.methods.find((m) => m.var.var === methodName);
    if (!methodBinding) {
    ...
    // new env extending the current one with fields binding and methods binding
    const fieldsEnv = makeExtEnv(proc.fields, proc.vals, proc.env);
    const methodsAndFieldsEnv = makeExtEnv(methodVars, methodArgs, fieldsEnv);
    //
    if (methodExp.body.length === 0) {
        return makeFailure("Method body cant be empty");
    }
    return evalSequence(methodExp.body, methodsEnvAndFieldsEnv);
};
///
