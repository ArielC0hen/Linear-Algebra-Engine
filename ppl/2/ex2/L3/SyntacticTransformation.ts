import { ClassExp, ProcExp, Exp, Program, makeProcExp, makeAppExp, makePrimOp, makeLitExp, makeIfExp, makeVarRef, Binding, CExp, makeVarDecl } from "./L3-ast";
    ...
    const ifs = (methods: Binding[]): CExp => methods.length === 0 
        ...
        : makeIfExp( 
            makeAppExp(
                ...
                [
                    makeVarRef("msg"),
                    makeLitExp(makeSymbolSExp(methods[0].var.var)) // binding -var-> var
                ]
            ),
            methods[0].val,
            ifs(methods.slice(1))
