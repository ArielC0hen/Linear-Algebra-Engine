...
export const expToPool = (exp: A.Exp): Pool => {
    const findVars = (e: A.Exp, pool: Pool): Pool =>
        ...
        A.isLitExp(e) && V.isCompoundSExp(e.val) ? (() => {
            const ListToArray = (val: SExp) : Exp[] => 
                isEmptySExp(val) ? [] :
                isCompoundSExp(val) ? [val.head].concat(ListToArray(val.tail))
                : [val];
            return reducePool(findVars, e.body, reducePoolVarDecls(extendPoolVarDecl, e.args, pool));
        }) ()
